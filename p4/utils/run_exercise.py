...
class ExerciseTopo(Topo):
    ...
    """
    def __init__(self, hosts, switches, links, log_dir, bmv2_exe, pcap_dir, cpu_port, **opts):
        ...
        for sw, params in switches.iteritems():
            if "program" in params:
                ...
                # add default switch
                switchClass = None
            self.addSwitch(sw, log_file="%s/%s.log" %(log_dir, sw), cls=switchClass, cpu_port=cpu_port)
...
class ExerciseRunner:
    ...
    def create_network(self):
        ...
        self.topo = ExerciseTopo(self.hosts, self.switches, self.links, self.log_dir, self.bmv2_exe, self.pcap_dir, self.cpu_port)
